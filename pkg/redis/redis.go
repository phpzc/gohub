...
//once 确保全局的 Redis对象 只实例一次
var once sync.Once

//Redis 全局 Redis，使用db1
...
//Set 存储key对应的value 且设置exiration过期时间
func (rds RedisClient) Set(key string, value interface{}, exiration time.Duration) bool {

	if err := rds.Client.Set(rds.Context, key, value, exiration).Err(); err != nil {
